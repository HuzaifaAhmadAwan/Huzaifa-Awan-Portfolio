...
TEST(PDBTestsPublic, test_bw_goal_aware)
{
    // 4 blocks
    ...
    std::vector<int> pattern = {
        domain.get_variable_hand_empty(),
        domain.get_variable_location(0)};

    auto task = create_task_from_domain(domain, initial_state, goal);
    auto hpdb = create_pdb_heuristic(task, pattern);

    ASSERT_EQ(get_initial_state_estimate(*task, *hpdb), 0);
}

TEST(PDBTestsPublic, test_bw_single_state)
{
    // 4 blocks
    BlocksWorld domain(4);
    ...
    std::vector<int> pattern = {
        domain.get_variable_hand_empty(),
        domain.get_variable_location(2),
        domain.get_variable_location(3)};
...
TEST(PDBTestsPublic, test_bw_single_state_reeval)
{
    // 4 blocks
    BlocksWorld domain(4);
...
TEST(PDBTestsPublic, test_bw_astar)
{
    // 6 blocks
    ...
    auto task = tests::create_task_from_domain(domain, initial_state, goal);
    std::shared_ptr hpdb = create_pdb_heuristic(task, pattern);
    auto engine = create_astar_search_engine(task, hpdb);
...
TEST(PDBTestsPublic, test_gripper_goal_aware)
{
    // 2 rooms, 2 balls
    Gripper domain(2, 2);
    ...
    std::vector<int> pattern = {
        domain.get_variable_ball_at(0),
        domain.get_variable_ball_at(1)};
...
TEST(PDBTestsPublic, test_gripper_single_state)
{
    // 2 rooms, 2 balls
    Gripper domain(2, 2);
...
TEST(PDBTestsPublic, test_gripper_single_state_reeval)
{
    // 2 rooms, 2 balls
    Gripper domain(2, 2);
...
TEST(PDBTestsPublic, test_gripper_astar)
{
    // 2 rooms, 2 balls
    Gripper domain(2, 2);
    ...
    ASSERT_TRUE(engine->found_solution());
    ASSERT_EQ(engine->get_statistics().get_reopened(), 0);
...
TEST(PDBTestsPublic, test_visitall_goal_aware)
{
    // 2 rooms, 2 balls
    VisitAll domain(2, 2);
    ...
    std::vector<int> pattern = {
        domain.get_variable_robot_at(),
        domain.get_variable_square_visited(1, 1)};
...
TEST(PDBTestsPublic, test_visitall_single_state)
{
    // 2 rooms, 2 balls
    VisitAll domain(2, 2);
    ...
    std::vector<int> pattern = {
        domain.get_variable_robot_at(),
        domain.get_variable_square_visited(1, 0),
        domain.get_variable_square_visited(1, 1)};
...
TEST(PDBTestsPublic, test_visitall_single_state_reeval)
{
    // 2 rooms, 2 balls
    VisitAll domain(2, 2);
...
TEST(PDBTestsPublic, test_visitall_astar)
{
    // 2 rooms, 2 balls
    VisitAll domain(2, 2);
    ...
    ASSERT_TRUE(engine->found_solution());
    ASSERT_EQ(engine->get_statistics().get_reopened(), 0);
    ASSERT_EQ(engine->get_statistics().get_expanded(), 7);
...
TEST(PDBTestsPublic, test_sokoban_goal_aware)
{
    ...
    std::vector<int> pattern = {
        domain.get_variable_player_at(),
        domain.get_variable_box_at(0),
        domain.get_variable_box_at_goal(0)};
...
TEST(PDBTestsPublic, test_sokoban_dead_end)
{
    const SokobanGrid playarea = {
        {' ', ' ', ' '},
        {' ', ' ', ' '},
...
TEST(PDBTestsPublic, test_sokoban_single_state)
{
    const SokobanGrid playarea = {
        {' ', 'G', ' '},
        {' ', ' ', ' '},
    ...
    std::vector<FactPair> initial(
        {domain.get_fact_player_at(0, 0),
         ...
         domain.get_fact_square_clear(0, 1, true),
         domain.get_fact_square_clear(0, 2, true),
         domain.get_fact_square_clear(0, 3, true),
         domain.get_fact_square_clear(1, 0, true),
         domain.get_fact_square_clear(1, 1, false),
         domain.get_fact_square_clear(1, 2, false),
         domain.get_fact_square_clear(1, 3, true),
         domain.get_fact_square_clear(2, 0, true),
         domain.get_fact_square_clear(2, 1, true),
         domain.get_fact_square_clear(2, 2, true),
         domain.get_fact_square_clear(2, 3, true)});
    ...
    std::vector<int> pattern = {
        ...
        domain.get_variable_box_at(0),
        domain.get_variable_box_at_goal(0),
        domain.get_variable_box_at(1),
        domain.get_variable_box_at_goal(1)};
...
TEST(PDBTestsPublic, test_sokoban_single_state_reeval)
{
    const SokobanGrid playarea = {
        {' ', 'G', ' '},
        {' ', ' ', ' '},
...
TEST(PDBTestsPublic, test_sokoban_astar)
{
    const SokobanGrid playarea = {
        {' ', ' ', ' '},
        {' ', ' ', 'G'},
    ...
    ASSERT_TRUE(engine->found_solution());
    ASSERT_EQ(engine->get_statistics().get_reopened(), 0);
    ASSERT_EQ(engine->get_statistics().get_expanded(), 13);
...
TEST(PDBTestsPublic, test_nomystery_goal_aware)
{
    // 0 -- 1 -- 2 -- 3
    ...
    pdbs::Pattern pattern = {
        domain.get_variable_truck_at(),
        domain.get_variable_package_at(0)};
...
TEST(PDBTestsPublic, test_nomystery_dead_end)
{
...
TEST(PDBTestsPublic, test_nomystery_single_state)
{
    // 4 -- 5 -- 6
    // |  /    /
    // | /    /
    ...
    NoMystery domain(7, 3, 1, roadmap);
    ...
    pdbs::Pattern pattern = {
        domain.get_variable_truck_at(),
        domain.get_variable_package_at(0),
        domain.get_variable_package_at(1),
        domain.get_variable_package_at(2)};
...
TEST(PDBTestsPublic, test_nomystery_single_state_reeval)
{
    // 4 -- 5 -- 6
    // |  /    /
    // | /    /
...
TEST(PDBTestsPublic, test_nomystery_astar)
{
    // 4 -- 5 -- 6
    // |  /    /
    // | /    /
    ...
    ASSERT_TRUE(engine->found_solution());
    ASSERT_EQ(engine->get_statistics().get_reopened(), 0);
    ASSERT_EQ(engine->get_statistics().get_expanded(), 205);
    ...
    const std::vector<OperatorID> expected_plan = {
        ...
        domain.get_operator_unload_package_id(6, 1, 1),
        domain.get_operator_drive_id(6, 5),
        domain.get_operator_load_package_id(5, 2, 0),
        domain.get_operator_drive_id(5, 6),
