...
TEST(CliquesHeuristicTestsPublic, test_bw_astar)
{
    ...
    pdbs::PatternCollection patterns = {
        ...
        {domain.get_variable_location(2)},
        {domain.get_variable_location(3)},
        {domain.get_variable_location(4)}};

    auto task = tests::create_task_from_domain(domain, initial_state, goal);
    std::shared_ptr hpdb_can = create_cliques_heuristic(task, patterns);
    auto engine = create_astar_search_engine(task, hpdb_can);

    engine->search();

    ASSERT_TRUE(engine->found_solution());
    ASSERT_EQ(engine->get_statistics().get_reopened(), 0);
    ASSERT_EQ(engine->get_statistics().get_expanded(), 586);
...
TEST(CliquesHeuristicTestsPublic, test_gripper_goal_aware)
{
    ...
    pdbs::PatternCollection patterns = {
        {domain.get_variable_robot_at(), domain.get_variable_ball_at(0)},
...
TEST(CliquesHeuristicTestsPublic, test_gripper_astar)
{
    ...
    ASSERT_TRUE(engine->found_solution());
    ASSERT_EQ(engine->get_statistics().get_reopened(), 0);
    ASSERT_EQ(engine->get_statistics().get_expanded(), 17);
...
TEST(CliquesHeuristicTestsPublic, test_visitall_goal_aware)
{
    ...
    pdbs::PatternCollection patterns = {
        {domain.get_variable_square_visited(0, 0)},
        {domain.get_variable_square_visited(0, 1)},
...
TEST(CliquesHeuristicTestsPublic, test_visitall_astar)
{
    ...
    const std::vector<OperatorID> expected_plan = {
        domain.get_operator_move_right_id(0, 0),
...
TEST(CliquesHeuristicTestsPublic, test_sokoban_goal_aware)
{
    ...
    pdbs::PatternCollection patterns = {
        {domain.get_variable_box_at(0), domain.get_variable_box_at_goal(0)},
        {domain.get_variable_box_at(1), domain.get_variable_box_at_goal(1)}
    };
...
TEST(CliquesHeuristicTestsPublic, test_nomystery_dead_end)
{
    ...
    pdbs::PatternCollection patterns = {
        {domain.get_variable_truck_at(), domain.get_variable_package_at(0)},
        {domain.get_variable_package_at(1)}
    };
...
TEST(CliquesHeuristicTestsPublic, test_nomystery_astar)
{
    ...
    ASSERT_TRUE(engine->found_solution());
    ASSERT_EQ(engine->get_statistics().get_reopened(), 0);
    ASSERT_EQ(engine->get_statistics().get_expanded(), 87);
